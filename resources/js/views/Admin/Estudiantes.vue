...
<v-card>
   ...
   <v-card-title><span class="headline">Agregar Estudiantessw</span></v-card-title>
   <v-card-text>
       <v-container>
...
export default {
    ...
    methods:{
        agregar(){
            ...
            axios.post('/Admin/Estudiante',nuevoEstudiante).then((res) =>{
       ...
       eliminar(index){
        const confirmacion = confirm(`Eliminar nota ${this.Estudiantes[index].name}`);
           if(confirmacion){
               ...
               axios.delete(`/Admin/Estudiante/eliminar/${this.Estudiantes[index].id}`).then(()=>{
